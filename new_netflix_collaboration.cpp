...
void modifiedprint(const table);
...
int HasItemAt(const table MyTable, int User,
              int Item);
table FillingBaseTable(table,string);
table FillingTestTable(table, string);
...
int main(int argc, const char * argv[])
{
    ...
    BaseTable = FillingBaseTable(BaseTable,"/Users/bghimire/workspace/neflix_prediction_algorithm/neflix_prediction_algorithm/u1base.txt");
    TestTable = FillingTestTable(TestTable,"/Users/bghimire/workspace/neflix_prediction_algorithm/neflix_prediction_algorithm/u1test.txt");
 ...
 *
 */
table FillingBaseTable(table BaseTable,string filepath){
        ...
        while ( getline (myfile,line))
        {
            if(parsing(line).size() > 0)
            {
                ...
                if (CurrentUser == Dummy[0]) {
                    ...
                    obj.item = Dummy[1];
                    obj.rating = Dummy[2];
                    while (obj.item > index) {
                        Dummyobj.user = Dummy[0];
                        Dummyobj.item = index;
                        Dummyobj.rating = -1;
                        MyRow.push_back(Dummyobj);
                        index++;
...
table FillingTestTable(table BaseTable,string filepath){
...
void modifiedprint(const table my_table){
    ...
    for (int i = 0; i < BaseTable.size(); i++) {
        for (int j  = 0; j < BaseTable.size(); j++) {
            if(i != j){
                ...
                cout<<"AvgOfActiveUser "<<i<<"  = "<<AvgOfActiveUser<<endl;
                cout<<"AvgOfCompareUser "<<j<<" = "<<AvgOfCompareUser<<endl;
...
double SimiCorrelation(const vector<ItemRating> ActiveUser,const vector<ItemRating> CompareToUser, double AvgOfActiveUser, double AvgOfCompareUser){
    ...
    while( (i < ActiveUser.size()) && (j < CompareToUser.size())){
        if(ActiveUser[i].item == CompareToUser[j].item  && (ActiveUser[i].item > 0)){
...
double CalSimiAvg(const vector<ItemRating> active_user, const vector<ItemRating> compare_user){
    ...
    while ((active_user.size() > i) && (compare_user.size() > j) ) {
        if((active_user[i].item == compare_user[j].item) && (active_user[i].item > 0)){
